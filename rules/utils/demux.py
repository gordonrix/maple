...
def validate_barcodes(barcode_info, reference_seqs):
    """
    Validate that all barcode contexts can be found in all reference sequences.

    Args:
        barcode_info: Dictionary of barcode information
        reference_seqs: Dictionary of {ref_name: ref_seq}

    Raises:
        ValueError: If context not found in any reference
    """
    ...
    for barcode_type, info in barcode_info.items():
        ...
        # Check that context exists in all references
        for ref_name, ref_seq in reference_seqs.items():
            if not re.search(pattern, ref_seq):
                raise ValueError(
                    f"Barcode context not found in reference '{ref_name}'.\n"
...
def demultiplex_bam(bam_path, output_dir, barcode_info, partition_groups,
                    label_groups, tag, references_csv,
                    threshold, screen_failures, screen_no_group, max_references=False):
    """
    ...
    Args:
        ...
        tag: Sample tag
        references_csv: CSV file with columns: ref_seq_name, ref_seq_alignment
        threshold: Minimum threshold for demultiplexing. If greater than 1, it is treated as an absolute count.
        screen_failures: Whether to screen failed barcodes
        screen_no_group: Whether to screen ungrouped files
        max_references: Maximum number of references to keep (False = keep all)
    ...
    reference_seqs = dict(zip(ref_df['ref_seq_name'], ref_df['ref_seq_alignment'].str.upper()))

    # Validate barcodes
    validate_barcodes(barcode_info, reference_seqs)
    ...
    # Determine which references to keep (if max_references is set)
    allowed_references = None
    if max_references:
        reference_counts = Counter()

        # Count all reads per reference
        for bam_entry in bam_file.fetch():
            reference_counts[bam_entry.reference_name] += 1

        # Get top N references
        top_refs = reference_counts.most_common(max_references)
        allowed_references = set(ref_name for ref_name, _ in top_refs)

        # Reset BAM file to beginning
        bam_file.close()
        bam_file = pysam.AlignmentFile(bam_path, 'rb')
    ...
    for bam_entry in bam_file.fetch():
        ...
        reference_seq = reference_seqs[bam_entry.reference_name]
        ...
        # Write to output file (skip if filtered by reference)
        if allowed_references is None or bam_entry.reference_name in allowed_references:
            output_files[output_prefix].write(bam_entry)

        # Update statistics - include reference name and label_output in the key
        # Order: tag, output_prefix, is_grouped, reference_name, ...barcode_names, label_output
        stats_key = tuple([tag, output_prefix, is_grouped, bam_entry.reference_name] + barcode_names + [label_output])
    ...
    if barcode_info:
        filter_output_files(stats_df, output_dir, tag, barcode_info,
                        threshold, screen_failures, screen_no_group)
...
def generate_statistics_dataframe(row_counts, barcode_info, tag, partition_groups, label_groups):
    ...
    columns = ['tag', 'output_file_barcodes', 'named_by_group', 'reference_name']
    ...
    for key, counts in row_counts.items():
        # key format: (tag, output_file_barcodes, named_by_group, reference_name, ...barcode_names, label_barcodes)
        # counts format: [barcodes_count, ...failure stats for each barcode type...]

        tag_val, output_file, is_grouped, ref_name = key[:4]
        num_barcodes = len(barcode_info)
        barcode_names = key[4:4+num_barcodes]  # Extract exact number of barcode names
        label_output = key[4+num_barcodes]     # Label output comes after barcode names

        row = [tag_val, output_file, is_grouped, ref_name]
    ...
    group_by_columns = ['tag', 'output_file_barcodes', 'named_by_group', 'reference_name', 'demuxed_count', 'label_barcodes']
...
def filter_output_files(stats_df, output_dir, tag, barcode_info,
    ...
    for _, row in stats_df.iterrows():
        ...
        if threshold > 1:
            ...
            threshold_absolute = threshold * total_sequences
        ...
        if screen_no_group and not row['named_by_group']:
            should_banish = True
...
def main():
    ...
    if 'snakemake' in globals():
        ...
        args = type('Args', (), {
            ...
            'references_csv': snakemake.params.references_csv,
            ...
            'screen_failures': snakemake.params.screen_failures,
            'screen_no_group': snakemake.params.screen_no_group,
            'max_references': snakemake.params.max_references
        })()
    else:
        # Running as standalone script
        parser = argparse.ArgumentParser(description=__doc__)
        parser.add_argument('--bam', required=True, help='Input BAM file')
        parser.add_argument('--references_csv', required=True, help='References CSV file with columns: ref_seq_name, ref_seq_alignment')
        ...
        parser.add_argument('--screen_failures', action='store_true',
                          help='Remove files with failed barcodes')
        parser.add_argument('--screen_no_group', action='store_true',
                          help='Remove files not assigned to a group')
        parser.add_argument('--max_references', type=int, default=False,
                          help='Maximum number of references to keep (False = keep all)')
    ...
    demultiplex_bam(
        ...
        label_groups, args.tag, args.references_csv,
        args.threshold, args.screen_failures, args.screen_no_group, args.max_references
    )

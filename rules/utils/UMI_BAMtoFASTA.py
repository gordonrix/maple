...
def main():
    ...
    outFA = snakemake.output[0]
    minimum = config['UMI_consensus_minimum']
    maximum = config['UMI_consensus_maximum']

    BAMs = BAMtoFASTA(tag, BAMin, logIn, outFA, minimum, maximum)
    BAMs.toFASTA()

class BAMtoFASTA:

    def __init__(self, tag, BAMin, logIn, outFA, minimum, maximum):
        """
        arguments:
    ...
    def toFASTA(self):
        ...
        with open(self.outFA, 'a') as output:

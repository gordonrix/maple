...
parser.add_argument('--exclude_indels', action='store_true', help='''Whether to use sequences with indels or not.
                        Default behavior is to include sequences that have indels, though indels will not influence the 
                        position 2D plot''')
...
if args.exclude_indels:
    data = data[data['NT_insertions'].isna()]
    data = data[data['NT_deletions'].isna()]
...
for seqIndex, seqType in enumerate(onehotArrayDict.keys(), start=1):
    ...
    onehot3DArray, genotypesDF = seq_array_from_genotypes(refSeq, data, seqType, onehot=True, exclude_indels=args.exclude_indels)

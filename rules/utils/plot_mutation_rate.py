...
def main():
    ...
    mutStatsCSV = pd.read_csv(str(snakemake.input.stats))
    timepointsCSV = pd.read_csv(str(snakemake.input.timepoints), header=1)
    ...
    if background:
        assert background in list(mutStatsCSV['barcode_group']), f'Provided barcode group for background subtraction, {background}, not present in {tag}_mutation-stats.csv'
        for _, row in mutStatsCSV[mutStats['barcode_group']==background].iterrows():
            backgroundRow = row
        backgroundBool = True
    else:
        backgroundRow = None
    ...
    for _, row in timepointsCSV.iterrows():
        ...
        timepointSamples = {} # dictionary to be populated with samples as generations:(tag, barcodeGroup) pairs

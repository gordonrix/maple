...
def find_barcode_position_in_alignment(ref_alignment, context):
    """
    Find barcode position in aligned sequence accounting for indels and IUPAC codes.
    ...
    import re
    pattern = ''
    for char in context.upper():
        if char in iupac_codes:
            pattern += '.'
        else:
            pattern += char
    
    match = re.search(pattern, ref_alignment)
    if not match:
        return None, 'context_not_present'
...
def identify_barcodes(bam_entry, reference_seq, barcode_info, barcode_dicts, hamming_dicts):
    ...
    for barcode_type, info in barcode_info.items():
        ...
        if isinstance(start, int):
            ...
            barcode_name = 'fail'
            failure_reasons[end] = 1
        
        sequence_barcodes[barcode_type] = barcode_name
        barcode_names.append(barcode_name)
        stats_list.extend([not_exact_match] + list(failure_reasons.values()))
...
def demultiplex_bam(bam_path, output_dir, barcode_info, partition_groups, 
    ...
    for bam_entry in bam_file.fetch(reference.id):
        ...
        if label_barcodes:
            ...
            # Remove label barcodes from sequence_barcodes for filename
            for bt in label_bc_types:
                sequence_barcodes.pop(bt, None)
        ...
        # Update statistics
        stats_key = tuple([tag, output_prefix, is_grouped] + barcode_names)
    ...
    stats_df = generate_statistics_dataframe(row_counts, barcode_info, tag)
...
def generate_statistics_dataframe(row_counts, barcode_info, tag):
    """
    ...
    group_by_columns = ['tag', 'output_file_barcodes', 'named_by_group', 'demuxed_count']
    sum_columns = {'barcodes_count': 'sum'}
    
    for barcode_type in barcode_info:
        columns.append(barcode_type)
        group_by_columns.append(barcode_type)
        
        failure_columns = [
            f'{barcode_type}:not_exact_match',
            f'{barcode_type}_failed:context_not_present',
            f'{barcode_type}_failed:barcode_not_in_fasta',
            f'{barcode_type}_failed:low_confidence'
        ]
        columns.extend(failure_columns)
    ...
    for key, counts in row_counts.items():
        row = list(key) + counts.tolist()
        rows.append(row)
    
    if not rows:
        rows = [[tag, 'all', False] + [0] * (len(columns) - 3)]

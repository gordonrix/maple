...
def main():
    ...
    BAMs = splitBAM(tag, BAMin, logIn, outDir, minimum, maximum, batches)
    BAMs.split()
...
class splitBAM:
    ...
    def __init__(self, tag, BAMin, logIn, outDir, minimum, maximum, batches):
        ...
        chunks          - number of files to split up BAM file into, based on modulo of UMI ID (BAM tag 'UG')
        """
        self.tag = tag
        self.BAMin = BAMin
        self.logIn = logIn
    ...
    def split(self):
        ...
        # open all BAM output files
        outBAMdict = {}
        for i in range(0, self.chunks):
            BAMoutName = f'{self.outDir}/batch{i}.bam'
            outBAMdict[i] = pysam.AlignmentFile(BAMoutName, 'wb', template=BAMin)
        ...
        for batch_index in range(0, UMI_groups_above_threshold['batch'].max()+1):
            ...
            for ID, UMIgroupBAMentries in UMI_BAMbatchDict.items():
                ...
                with pysam.AlignmentFile(os.path.join(self.outDir, f"UMI_{ID}_{row['final_umi']}_reads.bam"), 'wb', template=BAMin) as BAMout:
